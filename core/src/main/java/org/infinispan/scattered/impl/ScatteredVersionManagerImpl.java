...
public class ScatteredVersionManagerImpl<K> implements ScatteredVersionManager<K> {
   ...
   private CompletableFuture<Void> valuesFuture = new CompletableFutures().completedNull();
   ...
   @Override
   public synchronized void setSegmentVersion(int segment, long version) {
   ...
   public void notifyKeyTransferFinished(int segment, boolean expectValues) {
      ...
      CompletableFuture<Void> blockedFuture = blockedFutures.get(segment);
      if (blockedFuture != null) {
         blockedFuture.completeExceptionally(new CacheException("Max versions retrieval failed."));
      }
      if (trace) {
         if (expectValues) {
   ...
   @Override
   public void notifyValuesReceived(int topologyId) {
      for (int i = 0; i < numSegments; ++i) {
         do {
