...
//      return (command.getFlagsBitSet() & LOCAL_FLAGS) == 0;
   ...
   private InvocationStage retrieveSingleRemoteValue(InvocationContext ctx, Object key) {
      ...
      return makeStage(invokeNextThenApply(ctx, getCacheEntryCommand, (ctx1, command1, rv) -> {
         ...
         if (metadata != null && metadata.version() != null && svm.isVersionActual(keyPartitioner.getSegment(key), ((NumericVersion) metadata.version()).getVersion())){
            return null;
   ...
   protected Object handleWriteManyCommand(InvocationContext ctx, FlagAffectedCommand command, Collection<?> keys) throws Throwable {
      if (command.hasAnyFlag(FlagBitSets.COMMAND_RETRY)) {
         for (Object key : keys) {
            ctx.removeLookedUpEntry(key);
         }
      ...
      if (command.loadType() != VisitableCommand.LoadType.DONT_LOAD && canRetrieveRemoteValue(command)) {
         return prefetchKeysIfNeededAndInvokeNext(ctx, command, keys, true);
      } else {
         return invokeNext(ctx, command);
      }
   }
   ...
   public Object visitWriteOnlyManyEntriesCommand(InvocationContext ctx, WriteOnlyManyEntriesCommand command) throws Throwable {
      return handleWriteManyCommand(ctx, command, command.getAffectedKeys());
   }
