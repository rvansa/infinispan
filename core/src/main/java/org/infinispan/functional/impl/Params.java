...
public final class Params {
   ...
   public static class Externalizer implements AdvancedExternalizer<Params> {
      ...
      @Override
      public void writeObject(ObjectOutput output, Params object) throws IOException {
         for (Param param : object.params) {
            switch (param.serializeMethod()) {
               case NEVER:
                  break;
               case ORDINAL:
                  output.writeInt(((Enum) param).ordinal());
                  break;
               case OBJECT:
      ...
      public Params readObject(ObjectInput input) throws IOException, ClassNotFoundException {
         Param<?>[] params = new Param[DEFAULTS.length];
         for (int i = 0; i < params.length; ++i) {
            switch (DEFAULTS[i].serializeMethod()) {
               case NEVER:
                  params[i] = DEFAULTS[i];
                  break;
               case ORDINAL:
                  params[i] = (Param<?>) DEFAULTS[i].getClass().getEnumConstants()[input.readInt()];
                  break;
               case OBJECT:
                  params[i] = (Param<?>) input.readObject();
                  break;
            }
         }
