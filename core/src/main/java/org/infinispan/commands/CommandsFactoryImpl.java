...
public class CommandsFactoryImpl implements CommandsFactory {
   ...
   public void initializeReplicableCommand(ReplicableCommand c, boolean isRemote) {
      ...
      switch (c.getCommandId()) {
         ...
         case EntryProcessCommand.COMMAND_ID:
            EntryProcessCommand entryProcessCommand = (EntryProcessCommand) c;
            entryProcessCommand.init(notifier, configuration);
            break;
         default:
   ...
   public <K, V, T> EntryProcessCommand buildEntryProcessCommand(K key, EntryProcessor<K, V, T> processor, EnumSet<Flag> explicitFlags) {
      return new EntryProcessCommand(key, processor, false, notifier, explicitFlags);
   }
