...
public class NestedClassGenerator {
   ...
   private void generateCopyCtor() throws NotFoundException, CannotCompileException {
      ...
      if (!oldSuperclass.getName().equals(newSuperclass.getName())) {
         ...
         // we have to find appropriate superconstructor, but there may be no superctor without sideeffect
...
//                  int originalIndex = oldIterator.u16bitAt(readPc + 1);
//                  methodName = oldConstPool.getMethodrefName(originalIndex);
//                  String accessorName = "access$" + methodName;
//                  type = newConstPool.getMethodrefType(index);
//                  CtClass returnType = Descriptor.getReturnType(type, classPool);
//                  CtClass[] accessorParameters = Helper.prependClass(newInterceptor, Descriptor.getParameterTypes(type, classPool));
//                  int methodRef = newConstPool.addMethodrefInfo(newConstPool.addClassInfo(newInterceptor), accessorName, Descriptor.ofMethod(returnType, accessorParameters));
//                  newIterator.writeByte(Opcode.INVOKESTATIC, writePc);
//                  newIterator.write16bit(methodRef, writePc + 1);
//                  generateMethodAccessor(accessorName, returnType, accessorParameters, methodName, type);
//               }
//               break;
...
//   private void generateMethodAccessor(String accessorName, CtClass returnType, CtClass[] parameters, String methodName, String type) throws NotFoundException, CannotCompileException {
//      if (checkGenerated(newInterceptor, accessorName, parameters)) {
//         return;
//      }
//      CtMethod accessor = new CtMethod(returnType, accessorName, parameters, newInterceptor);
//      // flattened parent can be in different package than the inner class
//      accessor.setModifiers(Modifier.STATIC | Modifier.PUBLIC | Modifier.FINAL);
//      Bytecode bytecode = new Bytecode(newInterceptor.getClassFile().getConstPool());
//      bytecode.addLoadParameters(parameters, 0);
//      bytecode.addInvokespecial(newInterceptor, methodName, type);
