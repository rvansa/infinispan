...
class InterceptorGenerator {
   ...
   private static Log log = LogFactory.getLog(InterceptorGenerator.class);
   private final static AtomicLong classCounter = new AtomicLong();

   static final String REPLACE_TABLE = "$replaceTable$";
   static final String SYNC_ROOT = "$syncRoot$";
    ...
    */
   public void createClasses() throws NotFoundException, CannotCompileException {
   ...
   public void createFieldsAndConstructors() throws CannotCompileException, NotFoundException {
   ...
   public void createNestedClasses() throws NotFoundException, CannotCompileException, IOException, BadBytecode {
   ...
   public void createStaticVisitorMethods() throws NotFoundException, CannotCompileException {
    ...
    * @throws IOException
    */
   public void createMethods() throws NotFoundException, BadBytecode, CannotCompileException, IOException {
      ...
      // let's not copy the virtual version of these methods
   ...
   private CodeAttribute generateSynchronizationMethod(SpecificMethod method, CtClass newInterceptor, ConstPool newConstPool,
      ...
      CtMethod syncMethod = new CtMethod(returnType, newName + "$sync", parameterTypes, newInterceptor);
      ...
      syncBytecode.addInvokespecial(newInterceptor, newName + "$sync", returnType, parameterTypes);
...
//                  newIterator.writeByte(Modifier.isStatic(invoked.method.getModifiers()) ? Opcode.INVOKESTATIC : Opcode.INVOKESPECIAL, writePc);
               }
               newIterator.write16bit(ref, writePc + 1);
               break;
            case Opcode.LDC:
